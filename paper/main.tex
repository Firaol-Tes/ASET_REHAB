...
\IEEEauthorblockA{Higher Colleges of Technology\\
Dubai, UAE\\
% TODO: confirm Dr. Moez's email from the cc line of Dr. Yassine's message
}
\and
...
system builds on a new dataset of 243 sessions from 11 healthy
...
training restores cross-wrist accuracy to the within-wrist level. All
...
Eleven healthy adult university students (anonymized S01--S11)
volunteered for the recordings.
% TODO(Firaol): fill from collection records --- age range, gender
% split, dominant hand distribution.
Inclusion required being an adult with no current upper-limb injury,
pain, or movement restriction; participants with any musculoskeletal
complaint affecting the recorded movements were excluded, and all
recordings are labelled as performed with unaffected limbs. All
participants gave informed consent, and identities are anonymized in
all released code and results.
% TODO(Benachour): institutional ethics approval reference / exact
% consent-procedure wording.
...
sampling rates span 99.5--100.2~Hz with no missing samples.
...
session with no rest periods within a session.
% TODO(Firaol): confirm supervision arrangement during recording
% (instructed demonstration first? experimenter present?).
This gives 243 sessions and roughly 2.7~hours of motion. The property
central to this paper is that ten of the twelve exercises were
performed \emph{bilaterally with a watch on each wrist}: of 110
left/right session pairs, 108 overlap in absolute time --- synchronized
recordings of the same physical movement seen from both wrists (the
remaining two exercises are single-arm by design).
...
and the source of every figure and table in this paper are available
at \url{https://github.com/Firaol-Tes/ASET_REHAB}.
% TODO(Firaol): make the repository public before submission.
...
Computing, Networking, and Control in Cyber-Physical Systems}, 2026.
...
2009.
% TODO(Firaol): integrate Kedir et al. and Bona et al. DTW upper-limb
% IMU papers from Dr. Yassine's attached .bib (need the file).
\end{thebibliography}
