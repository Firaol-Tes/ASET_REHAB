...
clinical exercise recognition (a 33--40 percentage-point collapse) and
...
performed \emph{bilaterally with a watch on each wrist}. Ten
bilateral exercises across 11 participants would give 110 left/right
session pairs; one subject's wand-flexion left-wrist recording was
lost at collection, leaving 109, of which 107 overlap in absolute
time --- synchronized recordings of the same physical movement seen
from both wrists (the remaining two exercises of the twelve are
single-arm by design). The two non-overlapping pairs are left and
right recordings of the same exercise made minutes apart rather than
simultaneously, so they are excluded from every synchronized
analysis.
...
size. Removing the subject overlap is precisely what the next protocol
does. Under the strictest protocol --- unseen subject
\emph{and} unseen wrist --- mirror-augmented training achieves
92.5--93.2\%, against a within-wrist reference of 92.9\%. Because
...
cohort is predominantly right-handed and the dominant arm may be
performed differently. Neither affects the deployment claim, which is
