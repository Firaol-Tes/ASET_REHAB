...
with Core~ML on-watch deployment analysis~\cite{araya2025}, and
proposed a hybrid CNN--LSTM that, notably, closed its
...
personalized quality envelope exploits (Section~V-F). We adopt this
...
clinical outcome. Physical execution is intended to layer
admittance-controlled guidance over the collaborative arm's built-in
safety envelope --- reduced-speed mode and a tool-center-point contact
force limit (on the order of tens of newtons, well below injury
thresholds) --- so that the software trajectory is bounded by
hardware-enforced limits rather than trusted alone. Any patient
contact additionally requires ethics approval and a formal risk
assessment; both are deliberate future work.
