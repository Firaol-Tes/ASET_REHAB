...
Executed on the robot, the grader behaves consistently: reduced-range
and tremor executions are flagged on essentially every repetition with
the correct feature named, while clean executions pass. Truncated
executions are \emph{not} flagged --- the guarded execution protocol's
return-to-start blend completes every repetition by design, so
truncation cannot physically occur in guided mode; it remains
detectable (95\%) for free-moving patients.
% TODO: update the robot-grading sentence with final exp5b v2 numbers
